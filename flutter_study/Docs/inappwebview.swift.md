...
//
//    // 方向判定的阈值，避免极小抖动误判
//    private var horizontalBiasMinVelocity: CGFloat { 10 } // px/s
//    private var horizontalDominanceRatio: CGFloat { 1.2 } // |vx| >= 1.2 * |vy| 视为横向主导
//
...
//        else { return false } // 默认不与其他同时识别
//
//        // 取当前速度判断方向
//        let v = pan.velocity(in: self)
//        let absX = abs(v.x), absY = abs(v.y)
//
//        // —— 垂直：永远自己处理，不与外层同时识别（防止外层把竖向滚动拿走）——
//        if absY > absX && absY > horizontalBiasMinVelocity {
//            return false
//        }
//
//        // —— 横向：判断是否在左右边缘 ——
//        if absX >= max(horizontalBiasMinVelocity, horizontalDominanceRatio * absY) {
//            let sv = scrollView
//            let contentWidth = sv.contentSize.width
//            let frameWidth = sv.bounds.width
//            // 注意安全：浮点运算误差
//            let offsetX = sv.contentOffset.x
//            let maxOffsetX = max(0, contentWidth - frameWidth)
//            let epsilon: CGFloat = 0.5
//
//            let atLeftEdge = offsetX <= 0 + epsilon
//            let atRightEdge = offsetX >= maxOffsetX - epsilon
//            let canScrollHorizontally = contentWidth > frameWidth + epsilon
//
//            if canScrollHorizontally {
//                if atLeftEdge || atRightEdge {
//                    // 在边缘：允许与外层同时识别（让外层有机会拿到手势做切Tab）
//                    return true
//                } else {
//                    // 中间区域：不要同时识别（由WebView独占处理横向banner等）
//                    return false
//                }
//            } else {
//                // 没有横向可滚内容：允许外层识别，方便切Tab
//                return true
//            }
//        }
//
//        // 其它情况（低速/接近静止等）默认不同时识别，避免抖动
//        return false
//    }
//
//    // 提升外层切换Tab的优先级（仅当在边缘且我们允许同时识别时才会起作用）
//    public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer,
//                                  shouldBeRequiredToFailBy otherGestureRecognizer: UIGestureRecognizer) -> Bool {
//        // 这里返回 false，避免要求“外层失败后我再成功”，我们要优先保证WebView的滚动流畅
//        return false
//    }
//
//    public func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer,
//                                  shouldRequireFailureOf otherGestureRecognizer: UIGestureRecognizer) -> Bool {
//        // 同上，默认不强制谁失败，由 simultaneously 的策略决定
//        return false
//    }
//}
...
public class InAppWebView: WKWebView, UIScrollViewDelegate, WKUIDelegate,
    ...
    init(id: Any?, plugin: SwiftFlutterPlugin?, frame: CGRect, configuration: WKWebViewConfiguration,
        ...
        // 左边缘 → 上一页签
        edgePanLeft = UIScreenEdgePanGestureRecognizer(target: self, action: #selector(handleEdgePan(_:)))
        edgePanLeft.edges = .left
        edgePanLeft.delegate = self
        addGestureRecognizer(edgePanLeft)

        // 右边缘 → 下一页签
        edgePanRight = UIScreenEdgePanGestureRecognizer(target: self, action: #selector(handleEdgePan(_:)))
        edgePanRight.edges = .right
        edgePanRight.delegate = self
        addGestureRecognizer(edgePanRight)
    }
    ...
    @objc private func handleEdgePan(_ recognizer: UIScreenEdgePanGestureRecognizer) {
      ...
      if recognizer.state == .ended || recognizer.state == .cancelled {
        if recognizer == edgePanLeft {
          if translation.x > threshold {
              channelDelegate?.channel?.invokeMethod("onEdgeSwipe", arguments: ["direction": "left"])
          }
        } else if recognizer == edgePanRight {
          if translation.x < -threshold {
              channelDelegate?.channel?.invokeMethod("onEdgeSwipe", arguments: ["direction": "right"])
          }
        }
      }
    }
